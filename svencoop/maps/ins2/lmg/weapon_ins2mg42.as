...
string V_MODEL = "models/ins2/wpn/mg42/v_mg42.mdl";
...
class weapon_ins2mg42 : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase, INS2BASE::BipodWeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "saw", GetBodygroup(), (107.0/35.00) );
		}
		else
	...
	{
		if( WeaponADSMode == INS2BASE::IRON_IN )
			DeployBipod( BIPOD_IRON_IN, BIPOD_IRON_OUT, GetBodygroup(), (71.0/36.0), (57.0/38.0) );
		else
			DeployBipod( BIPOD_IN, BIPOD_OUT, GetBodygroup(), (71.0/36.0), (57.0/38.0) );
	}
	...
	void Reload()
	{
		...
		if( m_reloadTimer < g_Engine.time )
		{
			...
			{
				if( self.m_iClip >= 24 )
					Reload( MAX_CLIP, RELOAD, (137.0/18.71), SetBodygroup() );
				else if( self.m_iClip <= 0 )
					Reload( MAX_CLIP, RELOAD_EMPTY, (150.0/17.5), SetBodygroup() );
				else
					Reload( MAX_CLIP, RELOAD_HALF, (137.0/18.71), SetBodygroup() );
			}
			else if( WeaponBipodMode == INS2BASE::BIPOD_DEPLOYED )
				(self.m_iClip == 0) ? Reload( MAX_CLIP, BIPOD_RELOAD_EMPTY, (150.0/17.5), SetBodygroup() ) : Reload( MAX_CLIP, BIPOD_RELOAD, (137.0/18.71), SetBodygroup() );
			canReload = false;
		}
