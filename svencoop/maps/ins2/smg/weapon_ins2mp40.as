// Insurgency's MP-40
/* Model Credits
/ Model: Soul-Slayer (Edits by Norman The Loli Pirate)
/ Textures: Kimono, Norman The Loli Pirate (Edits)
...
// Models
string W_MODEL = "models/ins2/wpn/mp40/w_mp40.mdl";
string V_MODEL = "models/ins2/wpn/mp40/v_mp40.mdl";
string P_MODEL = "models/ins2/wpn/mp40/p_mp40.mdl";
string A_MODEL = "models/ins2/ammo/mags.mdl";
int MAG_BDYGRP = 6;
// Sprites
string SPR_CAT = "ins2/smg/"; //Weapon category used to get the sprite's location
// Sounds
string SHOOT_S = "ins2/wpn/mp40/shoot.ogg";
string EMPTY_S = "ins2/wpn/mp40/empty.ogg";
// Information
int MAX_CARRY   	= 150;
int MAX_CLIP    	= 32;
...
uint SLOT       	= 2;
uint POSITION   	= 9;
float RPM_AIR   	= 550; //Rounds per minute in air
float RPM_WTR   	= 450; //Rounds per minute in water
string AMMO_TYPE 	= "ins2_9x19mm";

class weapon_ins2mp40 : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	private array<string> Sounds = {
		"ins2/wpn/mp40/bltbk.ogg",
		"ins2/wpn/mp40/bltlck.ogg",
		"ins2/wpn/mp40/bltulk.ogg",
		"ins2/wpn/mp40/magin.ogg",
		"ins2/wpn/mp40/hit.ogg",
		"ins2/wpn/mp40/magout.ogg",
		"ins2/wpn/mp40/magrel.ogg",
		SHOOT_S,
		EMPTY_S
	};
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "mp5", GetBodygroup(), (52.0/30.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, (self.m_iClip > 0) ? DRAW : DRAW_EMPTY, "mp5", GetBodygroup(), (22.0/31.8) );
	}
	...
	void PrimaryAttack()
	{
		...
		ShootWeapon( SHOOT_S, 1, VecModAcc( VECTOR_CONE_1DEGREES, 2.25f, 0.8f, 1.75f ), (m_pPlayer.pev.waterlevel == WATERLEVEL_HEAD) ? 1024 : 8192, DAMAGE );
		...
		ShellEject( m_pPlayer, m_iShell, (WeaponADSMode != INS2BASE::IRON_IN) ? Vector( 32, 6.5, -7.1 ) : Vector( 32, 1.25, -3.55 ) );
	}
	...
	void Reload()
	{
		...
		{
			(self.m_iClip == 0) ? Reload( MAX_CLIP, RELOAD_EMPTY, (139.0/36.0), GetBodygroup() ) : Reload( MAX_CLIP, RELOAD, (90.0/36.0), GetBodygroup() );
			canReload = false;
		}
...
class MP40_MAG : ScriptBasePlayerAmmoEntity, INS2BASE::AmmoBase
{
	void Spawn()
	{
		Precache();
...
string GetAmmoName()
{
	return "ammo_ins2mp40";
}

string GetName()
{
	return "weapon_ins2mp40";
}

void Register()
{
	g_CustomEntityFuncs.RegisterCustomEntity( "INS2_MP40::weapon_ins2mp40", GetName() ); // Register the weapon entity
	g_CustomEntityFuncs.RegisterCustomEntity( "INS2_MP40::MP40_MAG", GetAmmoName() ); // Register the ammo entity
