...
string V_MODEL = "models/ins2/wpn/mk18/v_mk18.mdl";
...
class weapon_ins2mk18 : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	{
		if( !m_WasDrawn && self.m_iClip >= MAX_CLIP )
	...
	private int SetBodygroup()
	{
		...
		{
			if( (m_pPlayer.m_rgAmmo( self.m_iPrimaryAmmoType ) + self.m_iClip) >= MAX_CLIP )
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "m16", GetBodygroup(), (90.0/40.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, DRAW, "m16", GetBodygroup(), (22.0/27.0) );
	}
	...
	void PrimaryAttack()
	{
		...
		if( WeaponADSMode != INS2BASE::IRON_IN )
		{
			if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
				KickBack( 1.0, 0.3, 0.35, 0.03, 3.75, 2.75, 5 );
			else if( m_pPlayer.pev.velocity.Length2D() > 0 )
				KickBack( 0.85, 0.3, 0.3, 0.03, 3.75, 2.6, 7 );
			else if( m_pPlayer.pev.flags & FL_DUCKING != 0 )
				KickBack( 0.35, 0.2, 0.15, 0.02, 2.5, 1.25, 10 );
			else
				KickBack( 0.6, 0.225, 0.175, 0.0225, 2.6, 1.65, 8 );
		}
		else
		{
			PunchAngle( Vector( Math.RandomFloat( -2.85, -2.55 ), (Math.RandomLong( 0, 1 ) < 0.5) ? -0.50f : 0.50f, Math.RandomFloat( -0.5, 0.5 ) ) );
		}
	...
	void Reload()
	{
		...
		{
			(self.m_iClip == 0) ? Reload( MAX_CLIP, RELOAD_EMPTY, (136.0/37.0), SetBodygroup() ) : Reload( MAX_CLIP, RELOAD, (142.0/39.0), SetBodygroup() );
			canReload = false;
		}
...
class MK18_MAG : ScriptBasePlayerAmmoEntity, INS2BASE::AmmoBase
{
	void Spawn()
	{
		Precache();
