    ...
    if (g_SoundList.exists(soundArg)) {
      ...
      if (d < g_Delay) {
        ...
        if (soundArg == 'medic' || soundArg == 'meedic') {
         ...
         // }
         // else {
			float volume = 1.0f; // increased volume from .75 since converting stereo sounds to mono made them quiet
            g_SoundSystem.PlaySound(pPlayer.edict(), CHAN_AUTO, string(g_SoundList[soundArg]), volume, 0.4f, 0, g_Pitch.exists(steamId) ? int(g_Pitch[steamId]) : 100, 0, true, pPlayer.pev.origin);
         // }
        }
