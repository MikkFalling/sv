...
// Models
string W_MODEL = "models/ins2/wpn/mp5k/w_mp5k.mdl";
string V_MODEL = "models/ins2/wpn/mp5k/v_mp5k.mdl";
string P_MODEL = "models/ins2/wpn/mp5k/p_mp5k.mdl";
...
class weapon_ins2mp5k : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	private array<string> Sounds = {
		"ins2/wpn/mp5k/bltbk.ogg",
		"ins2/wpn/mp5k/bltlck.ogg",
		"ins2/wpn/mp5k/bltrel.ogg",
		"ins2/wpn/mp5k/magin.ogg",
		"ins2/wpn/mp5k/magout.ogg",
		"ins2/wpn/mp5k/magrel.ogg",
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "mp5", GetBodygroup(), (35.0/25.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, DRAW, "mp5", GetBodygroup(), (20.0/31.0) );
	}
	...
	void PrimaryAttack()
	{
		...
		ShootWeapon( SHOOT_S, 1, VecModAcc( VECTOR_CONE_1DEGREES, 1.9f, 0.75f, 1.6f ), (m_pPlayer.pev.waterlevel == WATERLEVEL_HEAD) ? 1024 : 8192, DAMAGE );
	...
	void Reload()
	{
		...
		{
			(self.m_iClip == 0) ? Reload( MAX_CLIP, RELOAD_EMPTY, (135.0/37.0), GetBodygroup() ) : Reload( MAX_CLIP, RELOAD, (95.0/37.0), GetBodygroup() );
			canReload = false;
		}
...
class MP5K_MAG : ScriptBasePlayerAmmoEntity, INS2BASE::AmmoBase
{
	void Spawn()
	{
		Precache();
...
string GetAmmoName()
{
	return "ammo_ins2mp5k";
}

string GetName()
{
	return "weapon_ins2mp5k";
}
