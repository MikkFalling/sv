...
const int GLUONGUN_MAX_AMMO			= 160;
const int GLUONGUN_MAX_CLIP 		= 20;
const int GLUONGUN_WEIGHT 			= 69;
const int GLUON_VELOCITY			= 1800;
...
class CGluongun : ScriptBasePlayerWeaponEntity
{
	...
	void Precache()
	{
		...
		g_Game.PrecacheGeneric( "sprites/custom_weapons/weapon_blue_rec3.txt" );
	...
	void PrimaryAttack()
	{
		...
		ShootGluon( basePlayer.pev, basePlayer.pev.origin + basePlayer.pev.view_ofs + g_Engine.v_forward * 0 + g_Engine.v_right * 0 + g_Engine.v_up * 0, g_Engine.v_forward * GLUON_VELOCITY , false , 255 );
		
		Math.MakeVectors( basePlayer.pev.v_angle + basePlayer.pev.punchangle );
		
		basePlayer.pev.punchangle.x -= 3;
		...
		self.m_flTimeWeaponIdle = WeaponTimeBase() + g_PlayerFuncs.SharedRandomFloat( basePlayer.random_seed,  0, 0 );
	...
	void TertiaryAttack()
	{
		...
		{
			self.PlayEmptySound( );
			self.m_flNextSecondaryAttack = WeaponTimeBase() + 24.55;
			self.m_flNextTertiaryAttack = WeaponTimeBase() + 15.55;
			self.m_flNextPrimaryAttack = WeaponTimeBase() + 8.55;
			return;
		}
		...
		if ( m_fInAttack == 0 )
		{
			...
			m_flAmmoStartCharge = WeaponTimeBase() + 0.2;
	...
	void StartFire()
	{
		...
		ShootGluon( basePlayer.pev, basePlayer.pev.origin + basePlayer.pev.view_ofs + g_Engine.v_forward * 12 + g_Engine.v_right * 3 + g_Engine.v_up * -8, g_Engine.v_forward * (m_iWastedAmmo*285) , true , m_iWastedAmmo );
		
		g_SoundSystem.EmitSoundDyn( basePlayer.edict(), CHAN_WEAPON, GLUONGUN_SOUND_FIRE2, 1.0, ATTN_NORM, 0, 100 );
		
		self.m_flNextSecondaryAttack = WeaponTimeBase() + 1.0;
		
		basePlayer.pev.punchangle.x -= 5;
	...
	void StartFire2()
	{
		...
		ShootGluon( basePlayer.pev, basePlayer.pev.origin + basePlayer.pev.view_ofs + g_Engine.v_forward * 12 + g_Engine.v_right * 3 + g_Engine.v_up * -8, g_Engine.v_forward * (m_iWastedAmmo*165) , true , m_iWastedAmmo );
		
		g_SoundSystem.EmitSoundDyn( basePlayer.edict(), CHAN_WEAPON, GLUONGUN_SOUND_FIRE2, 1.0, ATTN_NORM, 0, 100 );
		
		self.m_flNextSecondaryAttack = WeaponTimeBase() + 1.0;
		
		basePlayer.pev.punchangle.x = Math.RandomLong( -36, -35 );
	...
	void StartFire3()
	{
		...
		ShootGluon( basePlayer.pev, basePlayer.pev.origin + basePlayer.pev.view_ofs + g_Engine.v_forward * 12 + g_Engine.v_right * 3 + g_Engine.v_up * -8, g_Engine.v_forward * (m_iWastedAmmo*225) , true , m_iWastedAmmo );
		
		g_SoundSystem.EmitSoundDyn( basePlayer.edict(), CHAN_WEAPON, GLUONGUN_SOUND_FIRE2, 1.0, ATTN_NORM, 0, 100 );
		
		self.m_flNextSecondaryAttack = WeaponTimeBase() + 1.0;
		
		basePlayer.pev.punchangle.x = Math.RandomLong( -16, -15 );
	...
	void Reload()
	{
		...
		if ( basePlayer.m_rgAmmo( self.m_iPrimaryAmmoType ) == 2 ) // This is so you can't call this function with no Primary Ammo in reserve.
			return;
	...
	void WeaponIdle()
	{
		...
		switch( Math.RandomLong( 0, 1 ) )
		{
			...
			default: iAnim = GLUONGUN_IDLE2; break;	
		}
	...
	void ClipCasting( Vector origin, Vector angles )
	{
		...
		if (self.m_iClip == 0) // drop an additional clip if we're completely empty!
		{
			...
			NetworkMessage gluoncell2a(MSG_BROADCAST, NetworkMessages::SVC_TEMPENTITY);
				...
				gluoncell2a.WriteCoord(Math.RandomFloat(50,75)); // velocity
				gluoncell2a.WriteCoord(Math.RandomFloat(50,75)); // velocity
				gluoncell2a.WriteCoord(Math.RandomFloat(50,75)); // velocity
				gluoncell2a.WriteAngle(Math.RandomFloat(0,180)); // yaw
...
class DogeAmmo : ScriptBasePlayerAmmoEntity
{	
	void Spawn()
	{ 
		Precache();
	...
	{ 
		...
		if( pOther.GiveAmmo( iGive, "ammo_dogecoins", GLUONGUN_MAX_AMMO ) != -1)
		{
...
{
	return "weapon_blue_rec3";
}

void RegisterGluonGun()
{
	g_CustomEntityFuncs.RegisterCustomEntity( "CGluongun", GetGluonGunName() );
	g_ItemRegistry.RegisterWeapon( GetGluonGunName(), "custom_weapons", "ammo_dogecoins" );
}

void RegisterGluonAmmo()
{
	g_CustomEntityFuncs.RegisterCustomEntity( "DogeAmmo", "ammo_dogecoins" );
}