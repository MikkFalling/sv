...
const int GLUON_DAMAGE = Math.RandomLong(15,38); // for primary fire

const int GLUON_DAMAGE_ALT = Math.RandomLong(38,55); // for secondary fire
...
class CGluon : ScriptBaseEntity
{
	...
	void ExplodeTouch( CBaseEntity@ pOther )
	{
		...
		Vector vecSpot = self.pev.origin - pev.velocity.Normalize() * 8;
		Vector vecEnd = self.pev.origin + pev.velocity.Normalize() * 16;
		...
		g_WeaponFuncs.RadiusDamage(	self.pev.origin, self.pev, pevOwner, self.pev.dmg, 158, CLASS_NONE, DMG_ENERGYBEAM | DMG_NEVERGIB);
	...
	void ExplodeTouchAlt( CBaseEntity@ pOther )
	{
		...
		g_WeaponFuncs.RadiusDamage(	self.pev.origin, self.pev, pevOwner, self.pev.dmg, 156, CLASS_NONE, DMG_ENERGYBEAM | DMG_NEVERGIB);
