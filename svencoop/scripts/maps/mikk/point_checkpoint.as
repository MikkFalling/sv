...
class point_checkpoint : ScriptBaseAnimating
{
	...
	void Precache()
	{
		...
		if( string( self.pev.model ).IsEmpty() )
			g_Game.PrecacheModel( "models/mikk/cp/lambda.mdl" );
		else
			g_Game.PrecacheModel( self.pev.model );
		
		g_Game.PrecacheModel( "sprites/mikk/cp/exit1.spr" );
		
		g_SoundSystem.PrecacheSound( "mikk/cp.mp3" );
	...
	void Spawn()
	{
		...
		// Allow for custom models
		if( string( self.pev.model ).IsEmpty() )
			g_EntityFuncs.SetModel( self, "models/mikk/cp/lambda.mdl" );
		else
	...
	void Touch( CBaseEntity@ pOther )
	{
		...
		g_SoundSystem.EmitSound( self.edict(), CHAN_STATIC, "mikk/cp/cp.mp3", 1.0f, ATTN_NONE );
