...
class point_checkpoint : ScriptBaseAnimating
{
	...
	void Precache()
	{
		...
		if( string( self.pev.model ).IsEmpty() )
			g_Game.PrecacheModel( "models/mikk/op4/lambda.mdl" );
		else
			g_Game.PrecacheModel( self.pev.model );
		
		g_Game.PrecacheModel( "sprites/mikk/cp/exit1.spr" );
		
		g_SoundSystem.PrecacheSound( "mikk/op4/cp.mp3" );
		g_SoundSystem.PrecacheSound( "mikk/cp/beamstart7.wav" );
		g_SoundSystem.PrecacheSound( "mikk/cp/port_suckout1.wav" );
		
		if( string( self.pev.message ).IsEmpty() )
			self.pev.message = "mikk/cp/beamstart4.wav";
	...
	void Spawn()
	{
		...
		// Allow for custom models
		if( string( self.pev.model ).IsEmpty() )
			g_EntityFuncs.SetModel( self, "models/mikk/op4/lambda.mdl" );
		else
	...
	void Touch( CBaseEntity@ pOther )
	{
		...
		g_SoundSystem.EmitSound( self.edict(), CHAN_STATIC, "mikk/op4/cp.mp3", 1.0f, ATTN_NONE );
	...
	void RespawnStartThink()
	{
		...
		@m_pSprite = g_EntityFuncs.CreateSprite( "sprites/mikk/cp/exit1.spr", self.pev.origin, true, 10 );
		m_pSprite.TurnOn();
		m_pSprite.pev.rendermode = kRenderTransAdd;
		m_pSprite.pev.renderamt = 128;
	
		g_SoundSystem.EmitSound( self.edict(), CHAN_STATIC, "mikk/cp/beamstart7.wav", 1.0f, ATTN_NORM );
	...
	{
		g_SoundSystem.EmitSound( self.edict(), CHAN_STATIC, "mikk/cp/port_suckout1.wav", 1.0f, ATTN_NORM );
