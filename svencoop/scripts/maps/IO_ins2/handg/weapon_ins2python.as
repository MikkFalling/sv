// Insurgency's Colt Python
/* Model Credits
/ Model: Agressive Napkin, Norman The Loli Pirate (Speedloader, Edits), D.N.I.O. 071 (Cylinder Edits)
/ Textures: Teh Snake, Norman The Loli Pirate (Speedloader, Edits)
/ Animations: MyZombieKillerz, D.N.I.O. 071 (Major Edits)
/ Sounds: BtK Team, Navaro, Magmacow, KernCore (Conversion to .ogg format)
/ Sprites: D.N.I.O. 071 (Model Render), R4to0 (Vector), KernCore (.spr Compile)
/ Misc: KernCore (UV Chop, World Model UVs), D.N.I.O. 071 (Cylinder UVs, World Model UVs, Compile)
/ Script: KernCore
*/

#include "../base"

namespace INS2_PYTHON
{

// Animations
enum INS2_PYTHON_Animations
{
	IDLE = 0,
	DRAW_FIRST,
	DRAW,
...
// Models
string W_MODEL = "models/ins2/wpn/python/w_python.mdl";
string V_MODEL = "models/ins2/wpn/python/v_python.mdl";
string P_MODEL = "models/ins2/wpn/python/p_python.mdl";
string A_MODEL = "models/ins2/ammo/mags.mdl";
int MAG_BDYGRP = 48;
// Sprites
string SPR_CAT = "ins2/hdg/"; //Weapon category used to get the sprite's location
// Sounds
string SHOOT_S = "ins2/wpn/python/shoot.ogg";
string EMPTY_S = "ins2/wpn/python/empty.ogg";
// Information
int MAX_CARRY   	= 36;
int MAX_CLIP    	= 6;
...
uint SLOT       	= 1;
uint POSITION   	= 11;
float RPM_AIR   	= 0.15f; //Rounds per minute in air
float RPM_WTR   	= 0.22f; //Rounds per minute in water
float SHOOT_DELAY 	= 0.1; //Shooting delay
float EMPTY_DELAY 	= 0.13; //Shooting delay for dryfire
string AMMO_TYPE 	= "ins2_357magnum";

class weapon_ins2python : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	private array<string> Sounds = {
		"ins2/wpn/python/close.ogg",
		"ins2/wpn/python/dump1.ogg",
		"ins2/wpn/python/dump2.ogg",
		"ins2/wpn/python/hammer.ogg",
		"ins2/wpn/python/open.ogg",
		"ins2/wpn/python/speed.ogg",
		SHOOT_S,
		EMPTY_S
	};
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "python", GetBodygroup(), (70.0/30.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, DRAW, "python", GetBodygroup(), (16.0/30.0) );
	}
	...
	void ShootThink()
	{
		...
		ShootWeapon( SHOOT_S, 1, VecModAcc( VECTOR_CONE_1DEGREES, 3.25f, 0.5f, 1.65f ), (m_pPlayer.pev.waterlevel == WATERLEVEL_HEAD) ? 1024 : 8192, DAMAGE, false, DMG_SNIPER );

		m_pPlayer.m_iWeaponVolume = NORMAL_GUN_VOLUME;
		m_pPlayer.m_iWeaponFlash = BRIGHT_GUN_FLASH;

		PunchAngle( Vector( Math.RandomFloat( -4.00f, -3.50f ), (Math.RandomLong( 0, 1 ) < 0.5) ? -1.05f : 1.35f, Math.RandomFloat( -0.5, 0.5 ) ) );
	...
	void Reload()
	{
		...
		{
			Reload( MAX_CLIP, RELOAD, (136.0/35.0), SetBodygroup() );
			canReload = false;
		}
...
string GetAmmoName()
{
	return "ammo_ins2python";
}

string GetName()
{
	return "weapon_ins2python";
}

void Register()
{
	g_CustomEntityFuncs.RegisterCustomEntity( "INS2_PYTHON::weapon_ins2python", GetName() ); // Register the weapon entity
	g_CustomEntityFuncs.RegisterCustomEntity( "INS2_PYTHON::WEBLEY_MAG", GetAmmoName() ); // Register the ammo entity
