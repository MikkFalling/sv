...
const array<string> XEN_MAPS = { "hlcs_c14", "hlcs_c15", "hlcs_c16_a1", "hlcs_c16_a2", 
				 "hlcs_c16_a3", "hlcs_c16_a4", "hlcs_c17" };
	...
	else if( args.ArgC() == 3 && FirstArgChecker( args ) )
	{
		...
		if( g_BuyMenu.m_Items.length() > 0 )
		{
			...
			if( bItemFound )
			{
				...
				{
					g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CS16 BUYMENU] Not enough money (frags) - Cost: $" + uiCost + "\n" );
				}
				...
				g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CS16 BUYMENU] Invalid item: " + args.Arg(1) + "\n" );
		}
	}
...
void MoneyInit()
{
	...
	if( XEN_MAPS.find( g_Engine.mapname ) >= 0 )
...
final class BuyableItem
{
	...
	{
		if( uint(pPlayer.pev.frags) < m_uiCost )
		{
		...
		if( pPlayer.pev.frags <= 0 )
		{
			g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CS16 BUYMENU] Not enough money (frags) - Cost: $" + m_uiCost + "\n");
			return;
		}
		else if( uint(pPlayer.pev.frags) >= m_uiCost )
		{
